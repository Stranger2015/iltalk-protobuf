...
public class JavaPropsFormat {
   ...
   * {@code IntegerTerm.parseInt()}, this function recognizes the prefixes "0x"
        ...
        if (isSigned) {
          if (result > Integer.MAX_VALUE || result < Integer.MIN_VALUE) {
            throw new NumberFormatException(
              "NumberTerm out of range for 32-bit signed integer: " + text);
          }
        } else {
          if (result >= (1L << 32) || result < 0) {
            throw new NumberFormatException(
              "NumberTerm out of range for 32-bit unsigned integer: " + text);
          }
        }
      }
    } else {
      ...
      if (!isLong) {
        ...
        if (isSigned) {
          if (bigValue.bitLength() > 63) {
            throw new NumberFormatException(
              "NumberTerm out of range for 64-bit signed integer: " + text);
          }
        } else {
          if (bigValue.bitLength() > 64){
            throw new NumberFormatException(
                    "NumberTerm out of range for 64-bit unsigned integer: " + text);
          }
        }
      }
