package org.ltc.iltalk.com.declarativa.interprolog;
 ...
 * A SubprocessEngine object represents and gives access to a running Prolog process in background.
 * Multiple instances correspond to multiple Prolog processes, outside the Java Virtual Machine.
 */
abstract public class SubprocessEngine extends AbstractPrologEngine {
    ...
    protected OutputHandler stderrHandler; // stderrHandler will be null if outAndErrMerged==true
    ...
    protected Socket intSocket = null; // Used only for a particular way of interrupting Prolog
    String interruptCommand = null; // Used only for UNIX
    List<ClientRecognizer> listeners = new ArrayList<>();
    ...
    static class ClientRecognizer extends Recognizer implements RecognizerListener {
        ...
        public void recognized(Recognizer source, Object extra, String originStd) {
            ...
            if (originStd.equals(STDOUT)) {
                ...
                throw new IPException("Bad originStd:" + originStd);
            }
        }
    }
    ...
    public synchronized void removePrologOutputListener(PrologOutputListener client) {
        IntStream.range(0, listeners.size()).forEachOrdered(i -> {
            ClientRecognizer cr = listeners.get(i);
            if (cr.client == client) {
                listeners.remove(i);
        ...
        if (System.getProperty("java.version").compareTo("1.3") >= 0) {
            ...
            try {
                Method finalizeOnExit = System.class.getMethod("runFinalizersOnExit",
                        boolean.class);
                finalizeOnExit.invoke(null, new Boolean(true)); // for static methods first arg of invoke is ignored
            } catch (Exception e) {
    ...
    */
    protected void initSubprocess(String[] prologCommands) {
       ...
       =createProcess(prologCommands);
            // No explicit buffering, because it's already being done by our Process's streams
            // If not, OutputHandler will handle the issue
            stdoutHandler = new OutputHandler(.getInputStream(), (debug ? System.err : null), STDOUT)
            if (!outAndErrMerged) {
                stderrHandler = new OutputHandler(.getErrorStream(), (debug ? System.err : null), STDERR)
            }
            ...
            prologStdin = new PrintWriter(.getOutputStream())
    ...
    protected PrologOutputObjectStream buildPrologOutputObjectStream(OutputStream os) {
    ...
    /* (non-Javadoc)
     * @see AbstractPrologEngine#waitUntilAvailable()
     */
    @Override
                ...
                try {
                        ...
                        synchronized (this) {
                            ...
                            if (y != null) {
                                ...
                                if (y instanceof GoalToExecute) {
                                    ...
                                    ((GoalToExecute) y).setFirstGoalStatus(
                                            false); // Prolog sent us a message before this goal was sent via firstGoal...and that is carrying it over
                    ...
                    if (!shuttingDown) {
                        IPException toThrow = new PrologHaltedException("Prolog death detected in socket at setupCallbackServer, goal was " +
                                (goalsToExecute.isEmpty()
                                        ? "none!"
                                        : goalsToExecute.lastElement().getGoal().getGoal()),
                                e);
    ...
    public synchronized void shutdown() {
        ...
        try {
            ...
            throw new IPException("Problems closing sockets:" + e);
        }
        ...
        if (intServerSocket != null) {
            try {
                ...
                intSocket.close();
                intServerSocket.close();
            } catch (IOException e) {
                throw new IPException("Problems closing sockets:" + e);
            } finally {
                if (!shouldFire || !serverIsWindows()) .destroy(); // tries to avoid ugly messages on Windows
            }
            // Might there be a reason to send "halt" to Prolog assynchronously? Or to delay shutdown and send it synchronously?
            // Assuming not.
        } else if (!shouldFire || !serverIsWindows()) .destroy();
    ...
    protected int prepareInterrupt(String myHost, int prologPID) throws IOException { // requires successful startup steps
        ...
        if (serverIsWindows()) {
            ...
            interruptCommand = mustUseSocketInterrupt ? unixSimpleInterruptCommand(prologPIDstr) : unixInterruptCommand(prologPIDstr);
            return intPort;
        }
    }
    ...
    protected Object[] firstGoal(String G, String OVar, Object[] objectsP, String RVars) {
        ...
        try {
            ...
            if (shuttingDown) {
                throw new UnavailableResultException("Goal was " + G);
            } else {
        ...
        if (goalToDo.wasAborted()) {
            if (shuttingDown) {
                throw new UnavailableResultException("IP aborted goal was " + G);
            } else {
                throw new IPAbortedException(G + " was aborted");
            }
        }
        ...
        if (result.error != null) {
            if (result.error instanceof IPException) {
                throw (IPException) result.error;
            } else {
                if (result.error.equals(META_SYNTAX_ERROR)) {
                    ...
                    throw new IPPrologError(result.error /*+" in goal "+G*/);
                }
            }
        }
    ...
    protected synchronized boolean onlyFirstGoalSchedulled() {
        return isIdle() || (messagesExecuting.size() == 0 && goalsToExecute.size() == 1 &&
                goalsToExecute.elementAt(0).isFirstGoal());
    }
    ...
    public static class OutputDumper implements PrologOutputListener {
        ...
        public void print(String s) {
            try {
                if (available) {
                    w.write(s); // Do not flush, might affect performance measurement
                } else {
                    System.err.println("Lost output to file " + filename + ":" + s);
                }
            } catch (IOException ioe) {
